...
def main(config, case_config):
    ...
    for i in range(100):
        ...
        r_fp64 = torch.matmul(a_fp64, b_fp64)

        a = a.to(0)
        b = b.to(0)
        r_device = torch.matmul(a, b).cpu()
    ...
    latency_nowarm, latency_warm, cputime, kerneltime = do_test(
        torch.matmul, (a, b), host_device_sync, config, case_config)

    op2flops = lambda x: x * 2 * m * 1024 * 1024

    perf_result = cal_perf(cputime, kerneltime, op2flops,
                           case_config.SPECTFLOPS)
    print_result(config, "matmul", *perf_result, mape, mape_std,
