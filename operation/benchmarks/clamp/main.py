...
def main(config, case_config):
    ...
    for i in range(100):
        ...
        r_fp64 = torch.clamp(a_fp64, min=-0.5, max=0.5)

        a = a.to(0)
        r_device = torch.clamp(a, min=-0.5, max=0.5).cpu()
    ...
    latency_nowarm, latency_warm, cputime, kerneltime = do_test(
        torch.clamp, (a, min=-0.5, max=0.5 ), host_device_sync, config, case_config)

    op2flops = lambda x: x * m * 1024 * 1024

    perf_result = cal_perf(cputime, kerneltime, op2flops,
                           case_config.SPECTFLOPS)
    print_result(config, "clamp", *perf_result, mape, mape_std,
