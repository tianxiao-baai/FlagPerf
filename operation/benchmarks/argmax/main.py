...
def main(config, case_config):
    ...
    for i in range(100):
        ...
        r_fp64 = torch.argmax(a_fp64, 1)

        a = a.to(0)
        r_device = torch.argmax(a).cpu()
    ...
    op2flops = lambda x: x * m * n * 1024 * 1024 * 1024 * 1024

    perf_result = cal_perf(cputime, kerneltime, op2flops,
                           case_config.SPECTFLOPS)
    print_result(config, "argmax", *perf_result, mape, mape_std,
