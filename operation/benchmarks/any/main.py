...
def main(config, case_config):
    ...
    for i in range(100):
        a = torch.randn(2, m, dtype=dtype[config.dataformat]) < 0

        r_cpu = torch.any(a, 0)

        a = a.to(0)
        r_device = torch.any(a, 0).cpu() 
    ...
    a = torch.randn(2, m, 1024, 1024, dtype=dtype[config.dataformat]).to(0)
    a = a < 0

    latency_nowarm, latency_warm, cputime, kerneltime = do_test(
        torch.any, (a, 0), host_device_sync, config, case_config) # 调整为torch.sub

    op2flops = lambda x: x * 4 * m * 1024 * 1024 # 根据减法的实际FLOPs调整

    perf_result = cal_perf(cputime, kerneltime, op2flops,
                           case_config.SPECTFLOPS)
    print_result(config, "any", *perf_result, mape, mape_std,
